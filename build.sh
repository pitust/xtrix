...
rm build/*.o || true
ldc2 -mattr=-sse,-sse2,-sse3,-ssse3 -code-model=kernel -I src --relocation-model=static --gdwarf --oq --od build `find src -type f | grep '\.d'` -c --threads $(nproc) --betterC -mtriple=x86_64-elf
